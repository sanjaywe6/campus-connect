	...
	$lookups = [
		...
		'courses_table' => [
			'department' => [
				'parent_table' => 'departments_table',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`departments_table`.`id`) || CHAR_LENGTH(`departments_table`.`department_name`), CONCAT_WS(\'\', `departments_table`.`id`, \'-\', `departments_table`.`department_name`), \'\')',
				'parent_from' => '`departments_table` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'subject_table' => [
			'course_details' => [
				'parent_table' => 'courses_table',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`courses_table`.`id`) || CHAR_LENGTH(`courses_table`.`course_name`), CONCAT_WS(\'\', `courses_table`.`id`, \' ~ \', `courses_table`.`course_name`), \'\')',
				'parent_from' => '`courses_table` LEFT JOIN `departments_table` as departments_table1 ON `departments_table1`.`id`=`courses_table`.`department` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
