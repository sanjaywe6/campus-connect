...
*/
	...
	$lookups = [
		'students_table' => [
			...
			'course' => [
				'parent_table' => 'courses_table',
				'parent_pk_field' => 'id',
				'parent_caption' => '`courses_table`.`id`',
				'parent_from' => '`courses_table` LEFT JOIN `departments_table` as departments_table1 ON `departments_table1`.`id`=`courses_table`.`department` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
		...
		'subject_table' => [
			'course_details' => [
				'parent_table' => 'courses_table',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`courses_table`.`course_name`) || CHAR_LENGTH(if(`courses_table`.`starting_date`,date_format(`courses_table`.`starting_date`,\'%d/%m/%Y\'),\'\')), CONCAT_WS(\'\', `courses_table`.`course_name`, \' ~ \', if(`courses_table`.`starting_date`,date_format(`courses_table`.`starting_date`,\'%d/%m/%Y\'),\'\')), \'\')',
				'parent_from' => '`courses_table` LEFT JOIN `departments_table` as departments_table1 ON `departments_table1`.`id`=`courses_table`.`department` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'faculty_details' => [
				...
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`faculty_table`.`name`) || CHAR_LENGTH(`faculty_table`.`department`), CONCAT_WS(\'\', `faculty_table`.`name`, \' ~ \', IF(    CHAR_LENGTH(`departments_table1`.`department_name`) || CHAR_LENGTH(`departments_table1`.`hod`), CONCAT_WS(\'\',   `departments_table1`.`department_name`, \'-\', `departments_table1`.`hod`), \'\')), \'\')',
