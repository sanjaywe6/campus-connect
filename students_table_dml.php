...
function students_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$res = sql("SELECT `id` FROM `students_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `enrollment_table` WHERE `student_details`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="enrollment_table_view.php?filterer_student_details=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
		$RetMsg = str_replace(['[<TableName>]', '<TableName>'], sprintf($childrenATag, 'enrollment_table'), $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = `students_table_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = `students_table_view.php?SelectedID=' . urlencode($selected_id) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `students_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `results_table` WHERE `student_details`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="results_table_view.php?filterer_student_details=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
	...
	$res = sql("SELECT `id` FROM `students_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `attendance_table` WHERE `student_details`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="attendance_table_view.php?filterer_student_details=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
...
function students_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$filterer_course = Request::val('filterer_course');
	...
	<script>
		...
		AppGini.current_course__RAND__ = { text: "", value: "<?php echo addslashes($hasSelectedId ? $urow['course'] : htmlspecialchars($filterer_course, ENT_QUOTES)); ?>"};
			...
			$j("#course-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_course__RAND__.value, t: 'students_table', f: 'course' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							...
							$j('[id=course-container-readonly__RAND__]').html('<span class="match-text" id="course-match-text">' + resp.results[0].text + '</span>');
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'students_table', f: 'course' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php } else { ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_course__RAND__.value, t: 'students_table', f: 'course' },
				success: function(resp) {
					$j('[id=course-container__RAND__], [id=course-container-readonly__RAND__]').html('<span class="match-text" id="course-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=courses_table_view_parent]').hide(); } else { $j('.btn[id=courses_table_view_parent]').show(); }
	...
	if(!$fieldsAreEditable) {
		...
		$jsReadOnly .= "\t\$j('#course').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
		$jsReadOnly .= "\t\$j('#course_caption').prop('disabled', true).css({ color: '#555', backgroundColor: 'white' });\n";
	...
	$lookup_fields = ['department' => ['departments_table', 'Department'], 'course' => ['courses_table', 'Course'], ];
