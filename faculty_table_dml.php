...
// Data functions (insert, update, delete, form) for table faculty_table

// This script and data application was generated by AppGini, https://bigprof.com/appgini
// Download AppGini for free from https://bigprof.com/appgini/download/

function faculty_table_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('faculty_table');
	if(!$arrPerm['insert']) {
	...
	$data = [
		...
		'hire_date' => Request::dateComponents('hire_date', '1'),
	...
	$recID = tableInsert('faculty_table', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		faculty_table_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function faculty_table_copy_children($destination_id, $source_id) {
	global $Translation;
...
function faculty_table_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('faculty_table', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: faculty_table_before_delete
	if(function_exists('faculty_table_before_delete')) {
		$args = [];
		if(!faculty_table_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `faculty_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `subject_table` WHERE `faculty_details`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="subject_table_view.php?filterer_faculty_details=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
		$RetMsg = str_replace(['[<TableName>]', '<TableName>'], sprintf($childrenATag, 'subject_table'), $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = `faculty_table_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = `faculty_table_view.php?SelectedID=' . urlencode($selected_id) . (Request::val('Embedded') ? '&Embedded=1' : '') . '`;">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `faculty_table` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `timetable_table` WHERE `faculty_details`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="timetable_table_view.php?filterer_faculty_details=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
	...
	sql("DELETE FROM `faculty_table` WHERE `id`='{$selected_id}'", $eo);

	// hook: faculty_table_after_delete
	if(function_exists('faculty_table_after_delete')) {
		$args = [];
		faculty_table_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='faculty_table' AND `pkValue`='{$selected_id}'", $eo);
}

function faculty_table_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('faculty_table', $selected_id, 'edit')) return false;
	...
	$data = [
		...
		'phone' => Request::val('phone', ''),
		'address' => Request::val('address', ''),
		'hire_date' => Request::dateComponents('hire_date', ''),
		'department' => Request::lookup('department', ''),
		'designation' => Request::val('designation', ''),
	...
	// get existing values
	$old_data = getRecord('faculty_table', $selected_id);
	...
	// hook: faculty_table_before_update
	if(function_exists('faculty_table_before_update')) {
		$args = ['old_data' => $old_data];
		if(!faculty_table_before_update($data, getMemberInfo(), $args)) {
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="faculty_table_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}


	update_calc_fields('faculty_table', $data['selectedID'], calculated_fields()['faculty_table']);

	// hook: faculty_table_after_update
	if(function_exists('faculty_table_after_update')) {
		if($row = getRecord('faculty_table', $data['selectedID'])) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!faculty_table_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update record update timestamp
	set_record_owner('faculty_table', $selected_id);
}

function faculty_table_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	$allowUpdate = $hasSelectedId && check_record_permission('faculty_table', $selectedId, 'edit');
	$allowDelete = $hasSelectedId && check_record_permission('faculty_table', $selectedId, 'delete');
	...
	if($hasSelectedId && !check_record_permission('faculty_table', $selectedId, 'view'))
	...
	if(is_file(__DIR__ . '/hooks/faculty_table.gender.csv')) {
		$gender_data = addslashes(implode('', @file(__DIR__ . '/hooks/faculty_table.gender.csv')));
	...
	$combo_dob->MinYear = defined('faculty_table.dob.MinYear') ? constant('faculty_table.dob.MinYear') : 1900;
	$combo_dob->MaxYear = defined('faculty_table.dob.MaxYear') ? constant('faculty_table.dob.MaxYear') : 2100;
	...
	$combo_hire_date->DateFormat = "dmy";
	$combo_hire_date->MinYear = defined('faculty_table.hire_date.MinYear') ? constant('faculty_table.hire_date.MinYear') : 1900;
	$combo_hire_date->MaxYear = defined('faculty_table.hire_date.MaxYear') ? constant('faculty_table.hire_date.MaxYear') : 2100;
	...
	if($hasSelectedId) {
		if(!($row = getRecord('faculty_table', $selectedId))) {
			return error_message($Translation['No records found'], 'faculty_table_view.php', false);
		}
			...
			$j("#department-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_department__RAND__.value, t: 'faculty_table', f: 'department' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'faculty_table', f: 'department' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$templateDVP}") ? "./{$templateDVP}" : './templates/faculty_table_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$templateDV}") ? "./{$templateDV}" : './templates/faculty_table_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Faculty table details', $templateCode);
	...
	if(!$fieldsAreEditable) {
		...
		$jsReadOnly .= "\t\$j('#hire_date').prop('readonly', true);\n";
		$jsReadOnly .= "\t\$j('#hire_dateDay, #hire_dateMonth, #hire_dateYear').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
	...
	$templateCode = str_replace(
		'<%%COMBO(hire_date)%%>',
		(!$fieldsAreEditable ?
			'<div class="form-control-static">' . $combo_hire_date->GetHTML(true) . '</div>' :
			$combo_hire_date->GetHTML()
		), $templateCode);
	...
	if($hasSelectedId) {
		...
		$templateCode = str_replace('<%%VALUE(hire_date)%%>', app_datetime($row['hire_date']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(hire_date)%%>', urlencode(app_datetime($urow['hire_date'])), $templateCode);
	...
	$rdata = $jdata = get_defaults('faculty_table');
	if($hasSelectedId) {
		$jdata = get_joined_record('faculty_table', $selectedId);
		if($jdata === false) $jdata = get_defaults('faculty_table');
		$rdata = $row;
	}
	$templateCode .= loadView('faculty_table-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: faculty_table_dv
	if(function_exists('faculty_table_dv')) {
		$args = [];
		faculty_table_dv(($hasSelectedId ? $selectedId : FALSE), getMemberInfo(), $templateCode, $args);
	}
